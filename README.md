...
山药铭记是一款专为中医学习者设计的全栈学习平台，整合了伤寒论、方剂学、中药学、黄帝内经、金匮要略、温病学六大中医经典的学习资源。通过微信小程序提供移动端学习体验，后端基于 Node.js + Express + MySQL 提供 RESTful API，管理后台基于 Vue 3 实现内容与用户管理，项目主页基于 React 构建产品展示页面。平台致力于通过现代化的技术手段，让传统的中医学习变得更加便捷、高效、有趣。

### 核心特色

- **六大经典学习**：涵盖伤寒论、方剂学、中药学、黄帝内经、金匮要略、温病学，每科提供诵读、检查、抽查三种学习模式
- **随时背功能**：利用碎片时间进行背诵练习，支持按科目选择
- **背诵组功能**：按需展示内容，支持自定义组合
...
| 💊 方剂轻松过 | 快速记忆常用方剂组成与功效 | 诵读 → 检查 → 抽查 |
| 🌿 中药快快记 | 帮助记忆中药功效 | 诵读 → 检查 → 抽查 |
...
项目在 `云端数据ison/` 目录下包含六大中医经典的 JSON 数据文件：

| 数据集 | 文件 | 说明 |
|--------|------|------|
| 伤寒论 | shanghan.json | 《伤寒论》原文 |
| 方剂学 | fangji.json | 常用方剂数据库，含组成、功效、口诀 |
| 内经 | neijing.json | 《黄帝内经》核心内容 |
| 中药学 | zhongyao.json | 中药功效和记忆口诀 |
| 金匮要略 | jingui.json | 《金匮要略》原文 |
| 温病学 | wenbing.json | 温病学派经典原文 |

数据通过后台管理系统的**内容管理**模块导入和维护，支持在线编辑、发布和版本管理。
